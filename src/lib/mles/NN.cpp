...
#define LOAD_ARGS_VEC(act)                                                     \
  {                                                                            \
    stringstream ss;                                                           \
    int sz = act->getParametersSize();                                         \
    for (int j = 0; j < min(sz, (int)args.size()); j++) {                      \
      ss << args[j];                                                           \
      ss << " ";                                                               \
    }                                                                          \
    act->load(ss);                                                             \
...
void NN::verbose(bool isVerbose) { this->isVerbose = isVerbose; }
...
void NN::setDefaultActivationVec(string name, std::vector<double> &args) {
  defaultActivation = getActivation(name, true);
  LOAD_ARGS_VEC(defaultActivation);

  outputActivation = defaultActivation;
}

void NN::setInputSize(unsigned int size) { inputSize = size; }

void NN::setOutputSize(unsigned int size) { outputSize = size; }
...
void NN::setOutputActivationVec(string name, std::vector<double> &args) {
  outputActivation = getActivation(name, true);
  LOAD_ARGS_VEC(outputActivation);
}

void NN::setInputTransformation(const Eigen::VectorXd &a,
...
void NN::setOutputTransformation(const Eigen::VectorXd &a,
                                 const Eigen::VectorXd &b) {
  outputA = a;
  outputB = b;
...
Eigen::VectorXd NN::createInputVector() { return VectorXd(inputSize); }

Eigen::VectorXd NN::createOutputVector() { return VectorXd(outputSize); }
...
void NN::addLayerVec(unsigned int size, string name,
...
void NN::insertLayerVec(unsigned int pos, unsigned int size, string name,
                        std::vector<double> &args) {
  vector<unsigned int>::iterator it = layersSize.begin();
  advance(it, pos);
  layersSize.insert(it, size);
...
void NN::changeLayerVec(unsigned int pos, unsigned int size, string name,
                        std::vector<double> &args) {
  vector<unsigned int>::iterator it = layersSize.begin();
  advance(it, pos);
  *it = size;
...
TrainingResults NN::train(DataSet &trainingSet,
  ...
  if (trainingSet.getInputSize() != inputSize ||
      trainingSet.getOutputSize() != outputSize) {
  ...
  results.elapsedTime =
      chrono::duration_cast<chrono::microseconds>(elapsed).count() / 1000000.0;
...
void NN::build() {
  ...
  for (i = 1; i < sizes.size() - 1; i++) {
    layers.push_back(Layer(i - 1, sizes[i - 1] + 1, sizes[i] + 1,
                           activations[i - 1], false));
  }
...
DataSet NN::createDataSet() { return DataSet(inputSize, outputSize); }