...
DataSet::DataSet(unsigned int inputSize, unsigned int outputSize) {
  ...
  for (unsigned int i = 0; i < outputSize; i++) {
    ss.str("O");
    ss << i;
    this->outputHeader[i] = ss.str();
  }
}

DataSet::~DataSet() {}

unsigned int DataSet::getInputSize() { return inputSize; }

unsigned int DataSet::getOutputSize() { return outputSize; }
...
VectorXd DataSet::createInput() { return VectorXd(inputSize); }

VectorXd DataSet::createOutput() { return VectorXd(outputSize); }

VectorXd DataSet::createEntry() { return VectorXd(inputSize + outputSize); }
...
unsigned int DataSet::size() { return inputs.size(); }

VectorXd &DataSet::getInput(unsigned int pos) { return inputs[pos]; }

VectorXd &DataSet::getOutput(unsigned int pos) { return outputs[pos]; }
